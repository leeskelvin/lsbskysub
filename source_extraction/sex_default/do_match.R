...
out.output = cbind(
    X_OUTPUT = dat.output[,"X_IMAGE"]
    ,Y_OUTPUT = dat.output[,"Y_IMAGE"]
    ,MAG_OUTPUT = -2.5*log10(pmax(dat.output[,"FLUX_AUTO"],0)) + 27
    ,RAD_OUTPUT = pmax(dat.output[,"FLUX_RADIUS"],0) * 0.168 # arcsec
    ,ELLIP_OUTPUT = dat.output[,"ELLIPTICITY"]
    ,THETA_OUTPUT = dat.output[,"THETA_IMAGE"]
    ,A_OUTPUT = dat.output[,"A_IMAGE"]
    ,B_OUTPUT = dat.output[,"B_IMAGE"]
    ,AREA_OUTPUT = dat.output[,"ISOAREA_IMAGE"]
)
