...
for(i in 1:length(files)){
    ...
    nandat = scifits$dat[[1]]; nandat[maskdat==1] = NA
    write.fits(nandat, file="temp_masked.fits")
    
    # mask processing
    output = system(paste0(sex, " -c ../sex_default/default.sex -CATALOG_TYPE NONE -CHECKIMAGE_TYPE BACKGROUND,BACKGROUND_RMS -CHECKIMAGE_NAME temp_sky.fits,temp_std.fits temp_masked.fits 2>&1"), intern=T)
