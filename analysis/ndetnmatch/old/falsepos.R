...
pdfnames = paste0("falsepos-", c("a","b","a5","b5"), ".pdf")
...
for(i in 1:length(xdats)){
    ...
    for(j in 1:ncol(xdats[[i]])){
        ...
        if(j==3){labs = c(1,2)}
        if(j==4){labs = c(1,4)}
        aplot(NA, xlim=c(100,100000), ylim=c(0,1), xlab="", ylab="", las=1, side=NA, log="x", labels=labs, tick=FALSE, bty="n")
        abline(h=seq(0,1,by=0.2), col="grey75", lty=2, lend=1)
        abline(v=c(1,10,100,1000,10000,100000), col="grey75", lty=2, lend=1)
        aaxes(labels=labs, ynmin=1, las=1, xformat="p", mgp=c(2,0.5,0))
    ...
    mtext(side=2, line=2.5, text=bquote(paste("false positive fraction")), outer=T)
...
# merge into 1 PDF
system(paste("/usr/bin/gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dAutoRotatePages=/None -sOutputFile=falsepos.pdf falsepos-a.pdf falsepos-b.pdf"))
