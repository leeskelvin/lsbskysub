...
alabs = paste0("sky map quadrant range / ADU", c("","",rep(" (largest 5 matched sources only)",2)))
...
for(i in 1:length(xdats)){
    ...
    for(j in 1:ncol(xdats[[i]])){
        ...
        aaxes(side=1:3, labels=labs, ynmin=4, las=1, xformat="p")
...
# merge into 1 PDF
system(paste("/usr/bin/gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dAutoRotatePages=/None -sOutputFile=skylevel.pdf skylevel-a.pdf skylevel-b.pdf"))
