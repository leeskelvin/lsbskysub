...
ordera = c(1,5,3,7)
orderb = c(2,6,4,8)
yvals = list(dat$skymean[,ordera],dat$skymean[,orderb])
ystds = list(dat$skystd[,ordera],dat$skystd[,orderb])
...
for(i in 1:length(yvals)){
    ...
    par("oma"=c(5.5,4.5,3,4.5))
    
    # loop
    for(j in 1:ncol(yvals[[i]])){
        ...
        if(j==3){labs = c(2)}
        if(j==4){labs = c(4)}
        aplot(NA, xlim=c(1-0.25,nrow(yvals[[1]])+0.25), ylim=c(ylimlo[i],ylimhi[i]), xlab="", ylab="", las=1, side=NA, log="", labels=labs, tick=FALSE, bty="n")
        ...
        label("topright", lab=labtext, inset=c(2,0.5), outline="white")
        abox()
        
        # points
        ydat = suppressWarnings(fn(yvals[[i]][,j]))
        ydatlo = suppressWarnings(fn(yvals[[i]][,j] - (ystds[[i]][,j]) ))
        ydathi = suppressWarnings(fn(yvals[[i]][,j] + (ystds[[i]][,j]) ))
        ...
        apoints(1:nrow(yvals[[i]]), ydat, pch=metpch, lwd=lwd, cex=cex, lend=1, col=metcol)
#        if(length(toolow) > 0){
#            for(k in which(toolow)){
#                arrows(x0=(1:nrow(yvals[[i]]))[k], y0=ydat[k], y1=par("usr")[3], lwd=lwd, cex=cex, lend=1, col=metcol[k], length=0.075)
#            }
#        }
        
        if(j <= 2){
            par("xpd"=NA)
            apoints(1:nrow(yvals[[i]]), rep((par("usr")[3]+0.275),nrow(yvals[[i]])), pch=metpch, lwd=lwd, cex=cex, lend=1, col=metcol)
            par("xpd"=FALSE)
        }
